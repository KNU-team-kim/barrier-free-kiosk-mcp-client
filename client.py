        ...
        async with streamablehttp_client(url=MCP_SERVER_URL) as (read_stream, write_stream, get_session_id):
            async with ClientSession(
                ...
                messages: List[ChatCompletionMessageParam] = [
                    {"role": "system", "content": "당신은 AI 무인민원발급기 안내원입니다. 사용자의 요청을 분석하여, 필요하다면 제공된 Tool을 호출하고 그 결과를 바탕으로 사용자에게 친절하게 최종 답변을 생성해주세요. 지원하지 않는 서비스에 대한 문의는 명확하게 불가능하다고 답변해주세요."},
                ]
